...
├── [result]                # result 回调处理
└── [memory_query]          # memory_query 模式处理
...
| 前缀 | 来源函数 | 含义 | 典型输出示例 |
|------|---------|------|-------------|
| `[handle]` | `handle()` | 收到 Higo 的 HTTP 请求，记录请求模式 | `mode=probe sessionId=unknown` |
...
| `[transform]` | `_handle_transform()` | transform 模式处理过程 | `sessionId=xxx anchor=1/0 msg_count=N modelTokens=N` |
...
| 前缀 | 来源函数 | 含义 | 典型输出示例 |
|------|---------|------|-------------|
| `[ov_request]` | `request()` | 向 OpenViking 发出 HTTP 请求 | `GET/POST path agent=xxx body_len=N` |
| `[ov_response]` | `request()` | 收到 HTTP 成功响应（2xx） | `GET/POST path status=200 time=0.XXXs` |
| `[ov_error]` | `request()` | HTTP 请求失败（非 2xx、超时、网络异常） | `GET/POST path error=... time=0.XXXs` |
...
| 逻辑 | 文件 | 函数/类 |
|------|------|---------|
| 日志输出（入口层） | `main.py` | `handle()`, `_handle_probe()`, `_handle_transform()`, `_handle_result()` |
| 日志输出（引擎层） | `engine/openviking_engine.py` | `generate_memory()`, `_capture_messages()`, `_recall_memories()`, `_assemble_memory_text()`, `_maybe_commit()`, `compact()`, `capture_round_result()` |
| 日志输出（客户端层） | `engine/openviking_client.py` | `request()` |
| 诊断日志 | `engine/diagnostics.py` | `emit_diag()` |
